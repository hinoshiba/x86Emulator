...
func set_rm8(emu *Emulator, modrm *ModRM, value uint8) error {
	if modrm.mod == 3 {
...
func get_rm8(emu *Emulator, modrm *ModRM) (uint8, error) {
	if modrm.mod == 3 {
		return get_register8(emu, modrm.rm), nil
	}
	...
	return get_memory8(emu, address), nil
}
